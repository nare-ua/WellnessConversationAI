...
class CharDataset(Dataset):
    def __init__(self, chats, tok_path, vocab, max_len=32, wellness_answers=None):
        self._data = chats
        self.tokenizer = None
        self.first = True
        self.q_token = U_TKN
        ...
        self.tokenizer = nlp.data.SentencepieceTokenizer(tok_path, 0, 0)
        self.wellness_answers = wellness_answers
    ...
    def __getitem__(self, idx):
        ...
        q = turn['Q']
        if turn['type'] == "wellness":
            a = np.random.choice(self.wellness_answers[turn['label']])
        else:
            a = turn['A']

        #sentiment = str(turn['label'])
        q_toked = [self.q_token] + self.tokenizer(q) + [self.eos]
            #[self.sent_token] + self.tokenizer(sentiment) + [self.eos]
        q_len = len(q_toked)
        ...
        return (
            self.padder(self.vocab[q_toked + a_toked]),
            np.array(mask),
...
if __name__ == "__main__":
    if args.train:
        checkpoint_callback = ModelCheckpoint(
            filepath='model_chp/{epoch:02d}-{val_loss:.2f}',
            verbose=True,
            save_last=True,
            monitor='val_loss',
            mode='min',
